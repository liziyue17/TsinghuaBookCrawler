...
原来的代码不好用，我修改了几处，较好的解决了bug。

第一处：加入了”请输入书名“的代码。原来的代码书名是数字，不友好，不好用。

​		修改了这个bug之后，也解决了有些书只有一个章节，所以代码”`book_name = re.search(r'bookConfig.bookTitle="(\d+)"', s).group(1)`“会报错，实际上这里的书名就没有弄出来，但我也不会改，索性注释掉这个代码，换错要手动输入书名。

第二处：因为这个chapter（要输入的章节数）其实比较vague，就是说要输入的是几个链接的总数，并不是chapter章节是多少。因为有的书还有封面、附录等等，这些都算在chapter里面了。所以加上了try...except的代码。当然整个代码都可以优化，就是尝试一个比较大的chapter的值不断请求资源，如果返回404就缩小chapter的数值，不断二分；如果第一次没有404就应该将chapter扩大2倍直到返回404。这个代码逻辑应该可以自动找到这本书有多少个chapter，就不用每次都人工输入chapter的数值了。但是我不会检测返回的404，暂时搁置了。
